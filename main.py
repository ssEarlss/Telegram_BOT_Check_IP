...
async def cmd_start(message: types.Message):
    if helperJS.find("id", message.from_user.id, users) == None: return await message.reply(f"🔩 Ваш ID: {message.from_user.id}\n⛔ У вас отсутствует доступ.", reply_markup=types.ReplyKeyboardRemove())
...
def notifications():
    ...
    while True:
        ...
        if answerServers != "\n✅ Стабильно." and timer <= 0 or answerCameras != "\n✅ Стабильно." and timer <= 0:
            timer = 16
            with open(users, "r", encoding="utf-8") as file: usersfile = json.load(file)
            for user in usersfile: requests.get(f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={user['id']}&text=❗ Уведомление ❗\n💻 Сервера:{answerServers}\n\n🎥 Камеры: {answerCameras}")
        if timer > 0: timer -= 1
        time.sleep(5)

def ping(names):
    answer = ""
    with open(ipAdresses, "r", encoding="utf-8") as file: ipfile = json.load(file)
    for ip in ipfile[names]:
        ...
        if ping1.returncode == ping2.returncode != 0: answer += f"\n❌ {ip['ip']} [{ip['name']}] - Не работает"
    if answer == "": answer = "\n✅ Стабильно."
    return answer

async def main(): await dp.start_polling(bot, skip_updates=True)
def starts(): asyncio.run(main())
